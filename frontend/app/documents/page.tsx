...
export default function DocumentsPage() {
  ...
  const handleViewDetails = async (document: Document) => {
    ...
    // Charger l'historique quand on ouvre le modal
    await fetchDocumentHistory(document._id);
  };

  const fetchDocumentHistory = async (documentId: string) => {
    try {
      setLoadingHistory(true);
      const history = await documentsApi.getHistory(documentId);
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelectedDocument(null)}>
          ...
          >
            <CardHeader className="border-b border-slate-200/50 dark:border-slate-700/50">
              ...
              {/* Onglets */}
              <div className="flex gap-2 mt-4 border-b border-slate-200/50 dark:border-slate-700/50">
                <button
                ...
                <button
                  onClick={() => {
                    ...
                    if (documentHistory.length === 0 && selectedDocument) {
                      fetchDocumentHistory(selectedDocument._id);
                    }
                  }}
                  className={`px-4 py-2 text-sm font-medium transition-colors ${
                    activeTab === 'history'
                ...
                <div className="space-y-4">
                  ...
                  {loadingHistory ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-slate-600 dark:text-slate-400">Aucune modification enregistrée</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {documentHistory.map((entry: any, index: number) => {
                        ...
                        if (entry.oldValue && entry.newValue) {
                          const fields = ['firstName', 'lastName', 'cin', 'department', 'documentType', 'documentStatus'];
                          fields.forEach(field => {
                            const change = formatChange(field, entry.oldValue[field], entry.newValue[field]);
                        ...
                        return (
                          <div
                            key={entry._id || index}
                            className="border border-slate-200/50 dark:border-slate-700/50 rounded-lg p-4 bg-slate-50/50 dark:bg-slate-800/50"
                          >
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  <span className="px-2 py-1 text-xs font-semibold rounded bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">
                                    {entry.action === 'UPDATE_METADATA' ? 'Modification' : entry.action === 'CREATE_DOCUMENT' ? 'Création' : 'Suppression'}
                                  </span>
                                  <span className="text-xs text-slate-500 dark:text-slate-400">
                                ...
                                {entry.userId && typeof entry.userId === 'object' && (
                                  <p className="text-sm text-slate-700 dark:text-slate-300">
                                    Par: <span className="font-semibold">{entry.userId.firstName} {entry.userId.lastName}</span>
                                  </p>
                                )}
                                {entry.description && (
                                  <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">{entry.description}</p>
                                )}
                              </div>
                            </div>
                            ...
                            {changes.length > 0 && (
                              <div className="mt-3 pt-3 border-t border-slate-200/50 dark:border-slate-700/50">
                                <p className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">Changements:</p>
                                <div className="space-y-2">
                                  {changes.map((change, idx) => (
                                    <div key={idx} className="flex items-start gap-2 text-sm">
                                      <span className="font-medium text-slate-700 dark:text-slate-300 min-w-[100px]">
                                        {change.field === 'firstName' ? 'Prénom' :
                                         change.field === 'lastName' ? 'Nom' :
                                         change.field === 'cin' ? 'CIN' :
                                         change.field === 'department' ? 'Département' :
                                         change.field === 'documentType' ? 'Type' :
                                         change.field === 'documentStatus' ? 'Statut' : change.field}:
                                      </span>
                                      <div className="flex-1 flex items-center gap-2">
                                        ...
                                        <span className="px-2 py-1 rounded bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-xs font-semibold">
                                          {change.new}
                                        </span>
                                      </div>
                                    </div>
